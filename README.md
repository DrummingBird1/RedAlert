...
**עדכון (2026-07):** מצאנו מקור אחד אמיתי — עיריית תל אביב-יפו מפרסמת שכבת GIS ציבורית וזמינה (`gisn.tel-aviv.gov.il`, שכבה 592, ~374 מקלטים עם כתובת, סוג וסטטוס כשירות אמיתיים; ראו גם [opendata.tel-aviv.gov.il](https://opendata.tel-aviv.gov.il/)). האפליקציה שולפת ומטמיעה את הנתונים האלה אוטומטית (`GET /api/shelters/tel-aviv`, מטמון 24 שעות בצד השרת) **רק כשלא הוגדר `SHELTERS_URL`** — הם מחליפים את שתי הדוגמאות הכלליות של תל אביב במרקרים אמיתיים ומדויקים. **שאר הערים בארץ עדיין illustrative בלבד** — לא מצאנו (עדיין) שכבה עירונית מקבילה לכל רשות מקומית. אם יש לעירייה שלכם שכבת GIS פתוחה דומה (ArcGIS REST / WFS / CSV), אפשר להצביע אליה דרך `SHELTERS_URL` ולקבל את אותה רמת דיוק. דיוק מזויף מסוכן יותר מהיעדר דיוק באפליקציית בטיחות — לכן ההבחנה בין "אמיתי" ל"לדוגמה" נשארת חדה ומתועדת.
